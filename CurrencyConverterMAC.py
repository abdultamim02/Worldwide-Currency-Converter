...
class CurrencyConverter(wx.Frame):
    ...
    def App_Texts(self):
        ...
        self.Currency_Codes_Top.SetPosition(wx.Point(290, 20))                                                          # Set its position to be above the title
        ...
        self.Currency_Codes_Bottom.SetPosition(wx.Point(290, 120))                                                          # Set its position to be under the title
        ...
        self.Title_Label.SetPosition(wx.Point(290, 60))                                                          # Set its position to be at the top in between the the first and second currency codes
        ...
        self.First_Box = wx.Panel(self.panel, size = (322, 17), style = wx.SIMPLE_BORDER)                       # Initialize the First_Box label for a better feature to be around the amount input
        self.Horizontal_Boxsizer.Add(self.First_Box, 0, wx.EXPAND | wx.ALL, border = 10)                           # Add to Horizontal_Boxsizer
        self.First_Box.SetPosition(wx.Point(23, 200))                                                          # Set its position

        self.Second_Box = wx.Panel(self.panel, size = (348, 18), style = wx.SIMPLE_BORDER)                       # Initialize the Second_Box label for a better feature to be around the From_Currency_Choices
        ...
        self.Third_Box = wx.Panel(self.panel, size = (348, 18), style = wx.SIMPLE_BORDER)                       # Initialize the First_Box label for a better feature to be around the To_Currency_Choices
        ...
        Size_Font_Currently = wx.Font(15, family = wx.FONTFAMILY_ROMAN, weight = wx.FONTWEIGHT_BOLD, style = wx.FONTSTYLE_NORMAL)       # Size 12, Roman font, Bold wight, Normal style
        self.Currently.SetFont(Size_Font_Currently)

        # Set size and font for the copyright
        Size_Font_Copyright = wx.Font(12, family = wx.FONTFAMILY_DEFAULT, weight = wx.FONTWEIGHT_NORMAL, style = wx.FONTSTYLE_NORMAL)       # Size 12, Roman font, Bold wight, Normal style
        self.Copyright.SetFont(Size_Font_Copyright)
    ...
    def App_Amount_Input(self):
        ...
        self.Amount_input = wx.TextCtrl(self.panel, size = (285, 21))                                    # Create the Amount_input input to accepts integers for the Amount
    ...
    def Currency_Codes(self, evt):
        ...
        self.Code = wx.TextCtrl(self.panel, style = wx.TE_READONLY, size = (40, 21))                                    # Create the Code readonly input to accepts display the code of the currency chosen from
        ...
        for i in range(len(Currency_Symbols_)):
            if From_Currency_Symbol == Currency_Symbols_[i]:
                if len(Symbol_Codes[i]) == 3:
                    self.Code.SetValue(f"  {Symbol_Codes[i]}")
                elif len(Symbol_Codes[i]) == 2:
                    self.Code.SetValue(f"   {Symbol_Codes[i]}")
    ...
    def Convert_Button(self):
        ...
        self.Convert = wx.Button(self.panel, label = "Convert", size = (100, 25))                           # Create the convert button to convert from the first currency to the second
    ...
    def Swap_Button(self):
        ...
        self.Swap.SetPosition(wx.Point(733, 193))                                                     # Set its position
        ...
        Size_Font_Arrow = wx.Font(16, family = wx.FONTFAMILY_ROMAN, weight = wx.FONTWEIGHT_NORMAL, style = wx.FONTSTYLE_NORMAL)       # Size 25, Roman font, Normal wight, Normal style
...
def main():
    ...
    frame = CurrencyConverter(parent = None, title = "Currency Converter", size = (1150, 460))      # Initialize the frame to the CurrencyCoverter class
